...
class Calculator {
  ...
  updateDisplay() {
    this.currentOperandTextElement.innerText = this.formatAsNumber(
      this.currentOperand
    );
    if (this.operation !== undefined) {
      this.previousOperandTextElement.innerText = `${this.formatAsNumber(
        this.previousOperand
      )} ${this.operation}`;
    } else {
  ...
  formatAsNumber(numberAsString) {
    ...
    if (decimalDigitsAsString === undefined) {
      ...
      formattedString = `${integerDigits.toLocaleString(
        "en-us"
  ...
  animateButton(buttonValue) {
    const buttonPressed = allButtons.find(
      (button) => button.innerText === buttonValue
    );
